...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "SELECT p.*,d.*,(SELECT GROUP_CONCAT(a.value) FROM attributes a WHERE FIND_IN_SET (a.id, (SELECT GROUP_CONCAT(list_attribute_ids) FROM product_details d INNER JOIN products p ON p.id = d.product_id WHERE p.slug = ?1))) AS list_attribute_values FROM products p INNER JOIN product_details d ON d.product_id = p.id WHERE p.slug = ?1", nativeQuery = true)
