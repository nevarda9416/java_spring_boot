                        ...
                        children: [
                            {
                                ...
                                items: [
                                    ['Tất cả', 'show-all'],
                                    ['Desktop(pc) / Tablet', 'show-desktop hide-mobile'],
                                    ['Mobile', 'show-mobile hide-desktop']
                                ],
                                label: 'Hiển thị trên thiết bị <small style="font-size:85%">(Nếu không chọn mặc định vẫn sẽ hiển thị trên tất cả các thiết bị)</small>',
                                onChange: function () {
                                    var dialog = CKEDITOR.dialog.getCurrent();
                                    var inlineClassField = dialog.getContentElement('advanced', 'txtGenClass');
                                    inlineClassField.setValue(this.getValue());
                                },
                                setup: function (type, element) {
                    ...
                    elements: [
                        {
                            ...
                            label: 'Thêm mới',
                            onClick: function() {
                                window.open('/box/product', '_blank');
                            }
                        },
                        {
                            id: 'getProductImages',
                            type: 'button',
                            label: 'Lấy mới danh sách box (ảnh) sản phẩm',
                            onClick: function() {
                                $.ajax({
                                    type: 'GET',
                                    url: '/box/product/list/content',
                                    success: function (result, status, xhr) {
                                        var dialog = CKEDITOR.dialog.getCurrent(), listProductImages = dialog.getContentElement('product', 'listProductImages').getElement();
                                        listProductImages.setHtml(result);
                                        $('input[name="thumbnail_url"]').change(function () {
                                            if ($(this).prop('checked') === true) {
                                                var txtUrl = dialog.getContentElement('info', 'txtUrl');
                                                txtUrl.setValue($(this).val());
                                                var inlineClassField = dialog.getContentElement('advanced', 'txtGenClass');
                                                inlineClassField.setValue($(this).attr('class-name'));
                                                var device = dialog.getContentElement('info', 'device');
                                                device.setValue($(this).attr('class-name'));
                                            }
                                        });
                                    },
